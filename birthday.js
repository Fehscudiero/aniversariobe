...
function buildScene(){
  ...
  cx = W * (wide ? 0.58 : 0.5);
  cy = H * (wide ? 0.38 : 0.33);
  ry = Math.min(H * (wide ? 0.33 : 0.28), W * (wide ? 0.34 : 0.40));
  rx = ry * 1.16;
  groundY = H * 0.94;
...
function buildFilm(m){
  ...
  // --- the camera push -------------------------------------------------------
  // Duração estendida para garantir tempo de leitura confortável da mensagem
  t.fromTo(camera, { scale: 1.0, yPercent: 0 }, { scale: 1.08, yPercent: -1.5, duration: 4.8, ease: 'none' }, 1.38)
   .fromTo(fgrid, { xPercent: 0, yPercent: 0 }, { xPercent: -2.0, yPercent: -1.2, duration: 4.8, ease: 'none' }, 1.38);
  ...
  // --- the handoff bloom -----------------------------------------------------
  // A mensagem permanece visível e legível por mais de 3 segundos na tela
  t.to(barTop, { yPercent: -100, duration: 0.5, ease: 'power2.in' }, 5.50)
   .to(barBot, { yPercent: 100, duration: 0.5, ease: 'power2.in' }, 5.50)
   .set(bloom, { autoAlpha: 1 }, 5.60)
   .fromTo(bloom, { scale: 0.02 }, { scale: m.bloomScale, duration: 0.60, ease: 'power2.in' }, 5.60);
...
function resize(){
  ...
  if (played && filmTL && filmTL.progress() < 1){
